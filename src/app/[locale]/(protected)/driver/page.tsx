...
export default async function DriverDashboard({ params }: Props) {
    ...
    return <DriverContent userEmail={user?.email || 'Driver'} />;
}

function DriverContent({ userEmail }: { userEmail: string }) {
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="rounded-lg bg-green-100 p-4 text-sm text-green-800 dark:bg-green-900/30 dark:text-green-300">
                ...
                <p>✅ {tHealth('sessionStatus', { status: 'Authenticated as Driver' })}</p>
            </div>
        </div>
    );
}
